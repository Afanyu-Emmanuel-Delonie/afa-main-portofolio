      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12 py-12">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex gap-3">
            {[
              { icon: <Github className="w-4 h-4" />, target href: 'https://github.com/Afanyu-Emmanuel-Delonie' },
              { icon: <Linkedin className="w-4 h-4" />, href: '#' },
              { icon: <Twitter className="w-4 h-4" />, href: '#' },
            ].map((s, i) => (
              <a key={i} href={s.href} className="w-9 h-9 rounded-lg bg-zinc-900 border border-zinc-800 hover:border-[#F59E0B]/50 hover:text-[#F59E0B] text-zinc-400 flex items-center justify-center transition-all duration-300">
                {s.icon}
              </a>
            ))}
          </div>
